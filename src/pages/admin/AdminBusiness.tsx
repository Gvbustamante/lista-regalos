...
import { Btn, Pill, Ring } from '../../components/ui'
...
const input = 'w-full rounded-2xl border-2 border-mint-dark bg-white px-4 py-3 font-semibold text-ink outline-none focus:border-brand'
const label = 'grid gap-1 text-sm font-black text-brand/60'
...
export function AdminBusiness() {
  ...
  if (!data) return <p className="p-10 text-center font-bold text-brand/50">Cargando…</p>
  ...
  return (
    <div className="grid gap-5">
      <div className="flex flex-col gap-5 rounded-[32px] bg-mint p-5 md:flex-row md:items-center">
        <Link to="/admin" className="grid size-11 shrink-0 place-items-center rounded-full bg-white/70 text-xl font-black text-brand/60" aria-label="Volver">✕</Link>
        <Ring progress={limit ? 1 - Math.min(1, used / limit) : 1} color={limit && used / limit > 0.9 ? '#ef4444' : '#ffe11c'} size={120} stroke={16}>
          <span className="text-center font-black leading-tight text-brand">
            <span className="text-2xl">{used}</span>
            <br />
            <span className="text-xs text-brand/60">{limit ? `de ${limit}` : 'este mes'}</span>
          </span>
        </Ring>
        <div className="min-w-0 flex-1">
          <div className="text-sm font-black text-brand/60">Negocio</div>
          <h1 className="truncate text-3xl font-black text-brand">{biz.name}</h1>
      ...
      <div className="flex flex-wrap gap-2">
        {(['resumen', 'cuentas', 'tarifas', 'sesiones'] as Tab[]).map((t) => (
          <button key={t} onClick={() => setTab(t)} className={`rounded-full px-5 py-2 font-black capitalize ${tab === t ? 'bg-brand text-white' : 'bg-white text-brand'}`}>{t}</button>
        ))}
      </div>
...
function SummaryTab({ biz, plans, onSaved }: { biz: Business; plans: SaasPlan[]; onSaved: () => Promise<void> }) {
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-2">
      <section className="grid content-start gap-4 rounded-[28px] bg-mint p-5">
        <h2 className="text-lg font-black text-brand">Datos del negocio</h2>
        <label className={label}>Nombre<input className={input} value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} /></label>
        <div className="grid gap-4 sm:grid-cols-2">
          <label className={label}>Teléfono<input className={input} value={f.phone} onChange={(e) => setF({ ...f, phone: e.target.value })} /></label>
          <label className={label}>Moneda<input className={input} value={f.currency} maxLength={3} onChange={(e) => setF({ ...f, currency: e.target.value.toUpperCase() })} /></label>
        </div>
        <label className={label}>Dirección<input className={input} value={f.address} onChange={(e) => setF({ ...f, address: e.target.value })} /></label>
        <label className={label}>Notas internas (solo tú las ves)
          <textarea className={`${input} min-h-28`} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Pagos, acuerdos, contacto…" />
        </label>
      </section>

      <section className="grid content-start gap-4 rounded-[28px] bg-mint p-5">
        <h2 className="text-lg font-black text-brand">Suscripción</h2>
        <div className="grid grid-cols-2 gap-3">
          {plans.map((p) => (
            <button
              key={p.id}
              onClick={() => setF({ ...f, saas_plan: p.id })}
              className={`rounded-[22px] border-4 p-3 text-left ${f.saas_plan === p.id ? 'border-brand bg-brand-soft' : 'border-transparent bg-white'} ${p.active ? '' : 'opacity-50'}`}
            >
              <div className="font-black text-brand">{p.name}</div>
              <div className="text-sm font-bold text-slate-500">{money(p.price_monthly, p.currency)} / mes</div>
            </button>
          ))}
        </div>
        <label className={label}>Plan vence el (opcional)
          <input type="date" className={input} value={f.plan_expires_at} onChange={(e) => setF({ ...f, plan_expires_at: e.target.value })} />
        </label>
        <p className="text-sm font-bold text-brand/60">Creado el {new Date(biz.created_at).toLocaleDateString('es-CO')} · Enlace público /p/{biz.public_token.slice(0, 8)}…</p>
...
function MembersTab({ businessId, maxMembers }: { businessId: string; maxMembers: number | null }) {
  ...
  return (
    <section className="grid gap-4 rounded-[28px] bg-mint p-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-lg font-black text-brand">Cuentas del negocio</h2>
      ...
      <ul className="grid gap-2">
        {members.map((m) => (
          <li key={m.user_id} className="flex flex-wrap items-center gap-3 rounded-[22px] bg-white p-3">
            <span className="grid size-11 place-items-center rounded-full bg-brand-soft text-xl">👤</span>
            <div className="min-w-0 flex-1">
              <div className="truncate font-black text-ink">{m.name || m.email || m.user_id.slice(0, 8)}</div>
              <div className="truncate text-sm font-bold text-brand/60">{m.email} · desde {dateShort(m.created_at)}</div>
            </div>
            <select
              value={m.role}
              onChange={(e) => act(() => adminApi.setRole(businessId, m.user_id, e.target.value as Role))}
              className="rounded-full border-2 border-mint-dark bg-mint-soft px-3 py-2 font-bold text-brand"
            >
      ...
      >
        <input type="email" required className={`${input} min-w-60 flex-1`} placeholder="Correo de una cuenta registrada" value={email} onChange={(e) => setEmail(e.target.value)} />
        <select value={role} onChange={(e) => setRole(e.target.value as Role)} className="rounded-full border-2 border-mint-dark bg-white px-4 font-bold text-brand">
          {ROLES.map((r) => <option key={r.value} value={r.value}>{r.label}</option>)}
        </select>
  ...
  return (
    <section className="grid gap-4 rounded-[28px] bg-mint p-5">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-black text-brand">Tarifas del negocio</h2>
        <Btn onClick={() => edit()}>+ Nueva tarifa</Btn>
      </div>
      {(error || msg) && <p className="rounded-2xl bg-red-50 p-3 font-bold text-red-600">{error || msg}</p>}
      {draft && (
        <div className="grid gap-3 rounded-[22px] bg-white p-4 sm:grid-cols-[2fr_1fr_1fr]">
          <label className={label}>Nombre<input className={input} value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} /></label>
          <label className={label}>Minutos<input className={input} inputMode="numeric" value={draft.minutes} onChange={(e) => setDraft({ ...draft, minutes: e.target.value.replace(/\D/g, '') })} /></label>
          <label className={label}>Precio<input className={input} inputMode="numeric" value={draft.price} onChange={(e) => setDraft({ ...draft, price: e.target.value.replace(/[^\d.]/g, '') })} /></label>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {plans.map((p) => (
          <button key={p.id} onClick={() => edit(p)} className={`rounded-[22px] bg-white p-4 text-left hover:ring-4 hover:ring-brand-line ${p.active ? '' : 'opacity-50'}`}>
            <div className="flex items-center justify-between gap-2">
              <span className="font-black text-ink">{p.name}</span>
              {p.is_extension && <Pill>Extensión</Pill>}
            </div>
            <div className="text-2xl font-black text-brand">{duration(p.duration_minutes)}</div>
...
function SessionsTab({ biz }: { biz: Business }) {
  ...
  return (
    <section className="grid gap-4 rounded-[28px] bg-mint p-5">
      ...
      {loading && !data ? (
        <p className="font-bold text-brand/50">Cargando…</p>
      ) : (
        <ul className="grid gap-2">
          {(data?.sessions ?? []).map((s) => (
            <li key={s.id} className="grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-[18px] bg-white px-4 py-3 sm:grid-cols-[7rem_1fr_7rem_7rem_7rem]">
              <span className="font-bold text-brand/70">{dateShort(s.started_at)} {time(s.started_at)}</span>
              <span className="truncate font-black text-ink">{s.child_name}</span>
              ...
              <span className={`text-right font-black ${s.status === 'cancelled' ? 'text-slate-300 line-through' : 'text-brand'}`}>{money(paid.get(s.id) ?? 0, biz.currency)}</span>
            </li>
          ))}
          {data?.sessions.length === 0 && <li className="p-6 text-center font-bold text-brand/50">Sin entradas en este periodo</li>}
        </ul>
      )}
    </section>
  )
